...
public class Order {
    ...
    public void pay() {
        ...
        if (customer instanceof LoyaltyPoints lpCustomer) {
            double points = lpCustomer.getPoints();
            if (points > 0) {
                System.out.println(customer.getName() + " used " + points + " loyalty points to reduce the total.");
                total -= points;
                if (total < 0) total = 0;
        ...
        if (customer instanceof LoyaltyPoints lpCustomer2) {
            double earnedPoints = total * 0.10;
            lpCustomer2.addPoints(earnedPoints);
            System.out.println(customer.getName() + " earned " + earnedPoints + " loyalty points.");
        }
    }
